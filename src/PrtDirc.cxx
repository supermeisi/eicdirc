...
int main(int argc, char **argv) {
  ...
  TString macro, particle = "pi+", infile = "", lutfile = "", pdffile = "",
                 nnfile = "", outfile = "";
  int events(0), pdgid(0), geometry(1), firstevent(0), runtype(0), study(0),
      fid(0), verbose(0), batchmode(0), physlist(0), pmtLayout(2031),
      correction(2), field(0), ev(0), radiator(0), lensId(3), displayOpt(0);
  double momentum(0), theta(25), phi(0), beamZ(0), trackingres(0.0005),
      dark_noise(0), prismStepX(0), prismStepY, beamX(0), timeSigma(0.1),
      timeCut(0.5), testVal1(0), testVal2(0), testVal3(0);
  long seed = 0;
  ...
  for (int i = 1; i < argc; i = i + 2) {
    if (G4String(argv[i]) == "-m")
      ...
      lutfile = argv[i + 1];
    else if (G4String(argv[i]) == "-pdf")
      pdffile = argv[i + 1];
    else if (G4String(argv[i]) == "-nn")
      nnfile = argv[i + 1];
    else if (G4String(argv[i]) == "-field")
      field = atoi(argv[i + 1]);
    else if (G4String(argv[i]) == "-g")
      geometry = atoi(argv[i + 1]);
    else if (G4String(argv[i]) == "-ev")
      ev = atoi(argv[i + 1]);
    else if (G4String(argv[i]) == "-h")
      radiator = atoi(argv[i + 1]);
    else if (G4String(argv[i]) == "-theta")
      theta = atof(argv[i + 1]);
    else if (G4String(argv[i]) == "-phi")
      phi = atof(argv[i + 1]);
    else if (G4String(argv[i]) == "-b")
      batchmode = atoi(argv[i + 1]);
    else if (G4String(argv[i]) == "-f")
      firstevent = atoi(argv[i + 1]);
    else if (G4String(argv[i]) == "-e")
      events = atoi(argv[i + 1]);
    else if (G4String(argv[i]) == "-l")
      lensId = atoi(argv[i + 1]);
    else if (G4String(argv[i]) == "-x")
      particle = argv[i + 1];
    else if (G4String(argv[i]) == "-p")
      momentum = atof(argv[i + 1]);
    else if (G4String(argv[i]) == "-w")
      physlist = atoi(argv[i + 1]);
    else if (G4String(argv[i]) == "-r")
      runtype = atoi(argv[i + 1]);
    else if (G4String(argv[i]) == "-study")
      study = atoi(argv[i + 1]);
    else if (G4String(argv[i]) == "-fid")
      fid = atoi(argv[i + 1]);
    else if (G4String(argv[i]) == "-c")
      pmtLayout = atoi(argv[i + 1]);
    else if (G4String(argv[i]) == "-t1")
      testVal1 = atof(argv[i + 1]);
    else if (G4String(argv[i]) == "-t2")
      testVal2 = atof(argv[i + 1]);
    else if (G4String(argv[i]) == "-t3")
      testVal3 = atof(argv[i + 1]);
    else if (G4String(argv[i]) == "-gsx")
      prismStepX = atof(argv[i + 1]);
    else if (G4String(argv[i]) == "-gsy")
      prismStepY = atof(argv[i + 1]);
    else if (G4String(argv[i]) == "-gz")
      beamZ = atof(argv[i + 1]);
    else if (G4String(argv[i]) == "-gx")
      beamX = atof(argv[i + 1]);
    else if (G4String(argv[i]) == "-timeres")
      timeSigma = atof(argv[i + 1]);
    else if (G4String(argv[i]) == "-timecut")
      timeCut = atof(argv[i + 1]);
    else if (G4String(argv[i]) == "-trackingres")
      trackingres = atof(argv[i + 1]);
    else if (G4String(argv[i]) == "-v")
      verbose = atoi(argv[i + 1]);
    else if (G4String(argv[i]) == "-d")
      displayOpt = atoi(argv[i + 1]);
    else if (G4String(argv[i]) == "-dn")
      dark_noise = atof(argv[i + 1]);
    else if (G4String(argv[i]) == "-cor")
  ...
  if (outfile == "") {
    ...
    if (runtype == 0 || runtype == 10)
      outfile = "hits.root"; // simulation
  ...
  if (runtype == 2 || runtype == 3 || runtype == 4 || runtype > 19) {
    // PrtLutReco *reco = new PrtLutReco(infile, lutfile, pdffile, nnfile,
    // verbose);
  ...
  // physListFactory->GetReferencePhysList("QGSP_BERT"); for(auto v :
  // physListFactory->AvailablePhysLists()) std::cout << "v " << v << std::endl;
  ...
  if (physlist == 3) {
    ...
    // // const G4PTblDicIterator* particleIterator =
    // physicsList->GetParticleIterator(); auto* particleIterator =
