...
void PrtPrizmSD::Initialize(G4HCofThisEvent *hce) {
  ...
  fHitsCollection =
      new PrtPrizmHitsCollection(SensitiveDetectorName, collectionName[0]);
...
G4bool PrtPrizmSD::ProcessHits(G4Step *aStep, G4TouchableHistory *hist) {
  ...
  G4ThreeVector gnormal =
      theNavigator->GetLocalToGlobalTransform().TransformAxis(-normal);
  normal = touchable->GetHistory()->GetTransform(1).TransformAxis(
      gnormal); // in lDirc
  ...
  if (valid) {
    if (fRun->getEv() == 1) {
      if (vname == "wWedge") {
        ...
        if (normal.z() > 0.99)
          nid = 7; // front
        if (normal.z() < -0.5 && normal.z() > -0.999 && normal.x() > 0)
          nid = 8; // mirror
        if (normal.z() < -0.5 && normal.z() > -0.999 && normal.x() < 0)
          nid = 9; // pd
  ...
  // if (fRun->getRunType() == 5 && aStep->GetPreStepPoint()->GetPosition().z()
  // < 2100.100001) {
  //   nid = -5;
...
void PrtPrizmSD::EndOfEvent(G4HCofThisEvent *) {
  if (verboseLevel > 1) {
    ...
    G4cout << "\n-------->Prizm Hits Collection: in this event they are "
