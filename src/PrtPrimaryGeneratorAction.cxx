...
PrtPrimaryGeneratorAction::PrtPrimaryGeneratorAction()
  ...
  fParticleGun->SetParticlePosition(
      G4ThreeVector(0.0 * cm, 0.0 * cm, -0.5 * cm));
...
void PrtPrimaryGeneratorAction::GeneratePrimaries(G4Event *anEvent) {
  ...
  if (pdg != 0) {
    if (pdg == 2212)
      ...
      fPid = 8;
    else if (pdg == 10000 && fPid != 2)
      fPid = 2;
    else if (pdg == 10000)
      fPid = 0;
    else if (pdg == 10001 && fPid != 2)
      fPid = 2;
    else if (pdg == 10001)
      fPid = 1;
    else if (pdg == 10003 && fPid != 2)
      fPid = 2;
    else if (pdg == 10003)
      fPid = 3;
    else if (pdg == 10004 && fPid != 2)
      fPid = 2;
    else if (pdg == 10004)
      fPid = 4;
    else if (pdg == 10005 && fPid != 3)
      fPid = 3;
    else if (pdg == 10005)
      fPid = 4;
  ...
  if (fRunType == 0 || fRunType == 10 || fRunType == 5) { // simulation
    ...
    if (fRun->getPhi() >= 990 && fRun->getPhi() <= 999) {
      ...
      fTargetBarY =
          ((double)(kTargetBar - 5)) * fRadiatorW +
      ...
      double R = 1000. * (pt / CLHEP::GeV) / 0.3 /
                 fabs(Bval / CLHEP::tesla); // meters->mm
      if (fField == 0)
        R = 1E12;                           // no field
      ...
      double y2 =
          fTargetBarY; // target point !!!! HERE ASSUMES TEN BARS PER BOX
      double xa = (x2 - x1) / 2.;
      double ya = (y2 - y1) / 2.;
      double x0 = x1 + xa;
      double y0 = y1 + ya;
      ...
      double x4 = x0 - b * ya / a;
      double y4 = y0 + b * xa / a;
      // reverse order to get phi angle of perpendicular to ray from PV to
      // (x3,y3), NEG
    ...
    if (theta > 0 && theta < M_PI) {
      ...
      theta = acos((cos(25 * deg) - cos(160 * deg)) * G4UniformRand() +
                   cos(160 * deg));
      theta = M_PI - theta;
    ...
    if (fRun->getEv() == 1) {
      ...
      if (fRunType == 0)
        vec.rotateZ(0.016); // 0.016 hits the middle of the BaBar bar
    }
    fParticleGun->SetParticleMomentumDirection(vec);
  }
  ...
  if (fRunType == 1) { // LUT generation
    ...
    fParticleGun->SetParticlePosition(
        G4ThreeVector(-200, barShift, 0.5 * fRadiatorL + 630 - 0.2));
  ...
  // PrtManager::Instance()->getEvent()->setMomentum(TVector3(dir.x(), dir.y(),
  // dir.z()));
