...
PrtFastSimModelTracker::PrtFastSimModelTracker(G4String aModelName,
                                               G4Region *aEnvelope)
  ...
  double mombins[] = {0.75, 1.25, 1.75, 2.5,  3.5,  4.5,  5.5,
                      6.5,  7.5,  8.5,  9.5,  10.5, 11.5, 12.5,
                      13.5, 14.5, 15.5, 16.5, 17.5, 18.5, 19.5};

  double rtheta_0[] = {
      5.06794,  3.86015,  2.15975,  1.70208,  1.13201,  0.949914, 0.722017,
  ...
  double rphi_0[] = {18.6745,  12.426,   3.19723,  2.67958,  1.97109,  1.82281,
                     1.44746,  1.11069,  1.08232,  1.01937,  0.895427, 0.787919,
                     0.70771,  0.707664, 0.614802, 0.594137, 0.525847, 0.560032,
                     0.532071, 0.466807, 0.47712};
  double rphi_1[] = {6.54607,  3.47465,  2.92379,  2.04704,  1.48889,  1.17779,
                     0.912445, 0.812745, 0.697406, 0.629205, 0.544928, 0.495619,
                     0.460813, 0.444274, 0.402091, 0.362215, 0.361622, 0.334873,
                     0.328851, 0.301573, 0.29414};
  double rphi_2[] = {16,       11.1076,  5.4352,   3.55541,  2.73494,  2.07148,
                     1.98559,  1.49008,  1.38194,  1.26842,  1.14616,  1.02384,
                     1.01611,  0.943076, 0.871505, 0.746419, 0.735032, 0.68612,
                     0.670443, 0.631896, 0.575671};
...
G4bool PrtFastSimModelTracker::IsApplicable(
    const G4ParticleDefinition &aParticleType) {
  return aParticleType.GetPDGCharge() !=
         0; // Applicable for all charged particles
}

G4bool
PrtFastSimModelTracker::ModelTrigger(const G4FastTrack & /*aFastTrack*/) {
  return true; // No kinematical restrictions to apply the parametrisation
}

void PrtFastSimModelTracker::DoIt(const G4FastTrack &aFastTrack,
                                  G4FastStep &aFastStep) {

  // Calculate the final position (at the outer boundary of the tracking
  // detector) of the particle with the momentum at the entrance of the tracking
  // detector.
  ...
  G4double currentMinimumStep = 10.0 * m; // Temporary: change that to sth
                                          // connected to particle momentum.
  ...
  PrtManager::Instance()->getEvent()->setMomentum(
      TVector3(vmom.x(), vmom.y(), vmom.z()));

  // // use 0.5 mrad smearing for PDF
  // if(G4RunManager::GetRunManager()->GetCurrentEvent()->GetEventID() > 5000 &&
  // fTrackingRes < 0.0005) fTrackingRes = 0.0005;
  ...
  if (fTrackingRes < 1) {
    ...
    theta = 180 - theta / deg;
    dtheta = 0.001 * fTrMapTheta->GetBinContent(
                         fTrMapTheta->FindBin(0.001 * mom, theta));
    dphi = 0.001 *
           fTrMapPhi->GetBinContent(fTrMapPhi->FindBin(0.001 * mom, theta));
  }
...
double PrtFastSimModelTracker::get_res(TGraph *gg[3], double theta,
                                       double mom) {
